...
cat entire\_file.py                      grep \-n "function\_name" file.py

cat package.json                        jq '.dependencies' package.json

cat long\_log.txt                        tail \-50 long\_log.txt
...
cat file\_I\_just\_wrote.js                skip — you wrote it, you know it

reading a 500-line file for 1 function  sed \-n '45,60p' file.py

reading .env for one variable           grep "^API\_KEY=" .env
...
{agent\_id} | {status} | {result\_summary} | {next\_action}
...

{
  ...
  "content-generator": { "model": "gemma4", "tools": \["read","write"\], "prompt\_template": "content" },

  "email-writer": { "model": "gemma4", "tools": \["read","write"\], "prompt\_template": "email" },
...
├── tokens\_read: total tokens spent reading files

├── tokens\_written: total tokens in Pulse's outputs  

├── tokens\_wasted: tokens that didn't contribute to output (re-reads, filler, dead code)

├── read\_to\_output\_ratio: tokens\_read / tokens\_written (target: \< 2.0)

├── compaction\_score: lines\_of\_code\_removed / lines\_of\_code\_touched

├── agent\_prompt\_avg\_tokens: average across all agent prompts (target: \< 200\)

├── memory\_boot\_tokens: tokens consumed by session boot (target: \< 1000\)

└── efficiency\_grade: A/B/C/D/F based on above
...
3\. For each flag:
   ...
   \- Log: {file, before\_lines, after\_lines, tokens\_saved}
